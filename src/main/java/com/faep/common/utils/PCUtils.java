...
public class PCUtils
{
     ...
     * @return
     */
    public static String getPCIP(HttpServletRequest request) {
     ...
     *
     */
    public static String getPCMac(String sip) {
     ...
     * 而且137端口开放，则主机B会向主机A发送一个“UDP－NetBIOS－NS”应答包，即发Answer包给主机A。
     * 并利用UDP(NetBIOS Name Service)来快速获取远程主机MAC地址的方法
     *
     */
    public static class UdpGetClientMacAddr
    {
        ...
        public final DatagramPacket send(final byte[] bytes) throws IOException {
            DatagramPacket dp = new DatagramPacket(bytes, bytes.length, InetAddress.getByName(sRemoteAddr),
                    iRemotePort);
            ds.send(dp);
            return dp;
        }
